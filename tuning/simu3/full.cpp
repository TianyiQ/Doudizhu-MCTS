...
// 参数顺序：useHeu, useInf, c, SoD_B, SoD_E, iW_B, iW_E, sW, pW, u2P_E, infTL, def0
namespace PARA{ // 核心参数
	bool useHeuris=1,useInference=1;
	float c=4;
	float SoverD_B=25,SoverD_E=0.25; // B * E^{ratio_of_cards_played}
	float iniWeight_B=2./250,iniWeight_E=.001; // B * S * E^{ratio_of_cards_played}
	float structWeight=5,playedWeight=1; // weights in utility func
	ProbType utilityToProb_E=100; // E^{winProb/base}
	float inferenceTL=0.15;
	bool default0=0;
};
...
{
	// 参数顺序：useHeu, useInf, c, SoD_B, SoD_E, iW_B, iW_E, sW, pW, u2P_E, infTL, def0
	PARA::useHeuris = stoll(string(argc[1]));
	PARA::useInference = stoll(string(argc[2]));
	PARA::c = stold(string(argc[3]));
	PARA::SoverD_B = stold(string(argc[4]));
	PARA::SoverD_E = stold(string(argc[5]));
	PARA::iniWeight_B = stold(string(argc[6]));
	PARA::iniWeight_E = stold(string(argc[7]));
	PARA::structWeight = stold(string(argc[8]));
	PARA::playedWeight = stold(string(argc[9]));
	PARA::utilityToProb_E = stold(string(argc[10]));
	PARA::inferenceTL = stold(string(argc[11]));
	PARA::default0 = stoll(string(argc[12]));
	
	// std::cerr << PARA::useHeuris << PARA::useInference << PARA::c << PARA::SoverD_B << PARA::SoverD_E << PARA::iniWeight_B << PARA::iniWeight_E << PARA::structWeight << PARA::playedWeight << PARA::utilityToProb_E << PARA::inferenceTL << PARA::default0 << std::endl;
