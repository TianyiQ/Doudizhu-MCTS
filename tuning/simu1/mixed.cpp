...
// 参数顺序： useInf, c, SoD_B, SoD_E, sW, pW, u2P_E, infTL2, def0, thres
float infTL2=.15;
namespace PARA{ // 核心参数
	bool useHeuris;
	bool useInference=1;
	float c=5;
	float SoverD_B=25,SoverD_E=0.2; // B * E^{ratio_of_cards_played}
	constexpr float iniWeight_B=.0001/250,iniWeight_E=.0001; // B * S * E^{ratio_of_cards_played}
	float structWeight=5,playedWeight=1; // weights in utility func
	ProbType utilityToProb_E=100; // E^{winProb/base}
	float inferenceTL;
	bool default0=0;
};
...
constexpr float levelWeight_E = 1.23; // card of level i has weight E^i
		...
		for(int i=0;i<3;++i){
			...
			if(i == curPlayer && toBeat.second == i){
				...
				if(lev==level_joker && w==2){
					...
					float levelWeight = pow(levelWeight_E,
...
namespace SOL1{
	...
	CardCombo DetMCTS(State baseState,float rnd,float timeLeft){
		...
		const int TRIALS_PER_BATCH = 50, MAX_TRIALS = 100000;
...
//				if(detID%5 == 0) printf("%2d: %.10f\n",detID,prob);
			...
			root->assignState(startingState,1,PARA::useHeuris,0);
			
			float bestEsti = -100;
			int bestid = -1;
			for(int i=0;i<(int)root->sons.size();++i){
				if(root->sons[i].second->iniVal[root->curPlayer] > bestEsti){
					bestEsti = root->sons[i].second->iniVal[root->curPlayer];
					bestid = i;
				}
			}
			
			exScores[bestid] += expl(prob);
			
			#ifdef LOCAL_QTY
				assert((int)root->sons.size() == (int)options.size());
				assert(bestid!=-1);
				for(int i=0;i<(int)root->sons.size();++i){
		...
		debugInfo = to_string(cardRemaining[0] + cardRemaining[1] + cardRemaining[2]) + " "
				  ...
				  + to_string(lastProb) + " "
				  + to_string(log(*max_element(exScores.begin(),exScores.end())));
		#ifdef LOCAL_QTY
			printf("%d %d\n",DetDone,totPlayouts);
		#endif
...
{
	// 参数顺序： useInf, c, SoD_B, SoD_E, sW, pW, u2P_E, infTL, def0, thr
	PARA::useInference = stoll(string(argc[1]));
	PARA::c = stold(string(argc[2]));
	PARA::SoverD_B = stold(string(argc[3]));
	PARA::SoverD_E = stold(string(argc[4]));
	PARA::structWeight = stold(string(argc[5]));
	PARA::playedWeight = stold(string(argc[6]));
	PARA::utilityToProb_E = stold(string(argc[7]));
	infTL2 = stold(string(argc[8]));
	PARA::default0 = stoll(string(argc[9]));
	int thr = stoll(string(argc[10]));
	...
	if (stage == Stage::BIDDING)
	{
		...
		CardCombo res = SOL1::DetMCTS(baseState,0,0.925-(curTime-baseTime));
	...
	else if (stage == Stage::PLAYING)
	{
		...
		#ifdef LOCAL_QTY
			printf("have %d cards\n",cardRemaining[myPosition]);
		#endif
		if(cardRemaining[myPosition] >= thr)
			myAction = SOL1::DetMCTS(baseState,curRnd,0.925-(curTime-baseTime));
		else
			myAction = SOL2::DetMCTS(baseState,curRnd,0.925-(curTime-baseTime));
